...
import { newGroupChat } from "./chatController.js";
...
export const register = async (req, res) => {
    try {
        ...
        const defaultAvatar = process.env.DEFAULT_AVATAR_URL || "https://example.com/default/avatar.jpg";
        const defaultCoverImage = process.env.DEFAULT_COVER_URL || "https://example.com/default/cover.jpg";

       
        const avatarPath = avatar && typeof avatar === "object" && avatar.url ? avatar.url : (typeof avatar === "string" ? avatar : "");
        const coverImagePath = coverImage && typeof coverImage === "object" && coverImage.url ? coverImage.url : (typeof coverImage === "string" ? coverImage : "");

       
        const myCloudAvatar = avatarPath
            ? await cloudinary.v2.uploader.upload(avatarPath, {
                folder: "avatars",
                width: 150,
                crop: "scale",
            })
            : { public_id: "default_avatar_id", secure_url: defaultAvatar };

      
        const myCloudCover = coverImagePath
            ? await cloudinary.v2.uploader.upload(coverImagePath, {
                folder: "covers",
                width: 800,
                crop: "scale",
            })
            : { public_id: "default_cover_image_id", secure_url: defaultCoverImage };
